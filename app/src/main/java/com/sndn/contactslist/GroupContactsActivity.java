...
public class GroupContactsActivity extends AppCompatActivity {
    ...
    private void initData() {
        ...
        list.add(new User("……%￥#￥%#"));
        list.add(new User("啊海"));
        Collections.sort(list); // 对list进行排序，需要让User实现Comparable接口重写compareTo方法
        SortAdapter adapter = new SortAdapter(this, list);
        listView.setAdapter(adapter);

        LinearLayout linearLayout = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.group_members_list_head, null, false);
        LinearLayout item = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.group_members_list_head_item, linearLayout, false);
        LinearLayout item2 = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.group_members_list_head_item, linearLayout, false);
